2026.02.15
1. Feature (Design Mode): added `join` to recombine two CSVs/DataFrames on `ID` and reconcile parallel branch outputs back into a single design table (backbone column order preserved; only new columns from `other_data` are inserted; ambiguity resolved via `join_policy`).
