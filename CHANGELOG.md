2026.02.18
1. Bugfix (Index): removed leading space from `' barcodes_unique'` stats key → `'barcodes_unique'`.
2. Bugfix (Index): `vars` in computing-index stats path was `None` → `{}`.
3. Bugfix (Acount): renamed stats key `callback_false` → `callback_false_reads` to match `xcount` and the `_reads` suffix convention.
4. Bugfix (Verify): restored stats `step_name` to the gerund form `'scanning-oligo-pool'` for consistency with other modules.
5. Bugfix (Compress): moved `random_seed` assignment before `stamp_stats` call, consistent with all other modules.
6. Bugfix: `vars` in early-exit stats paths was `None` → `{}` across `barcode`, `primer`, `motif`, `spacer`, `pad`, `index`, `inspect`.
7. Docs (`docs/api.md`): added Stats Dict Schema section to Appendix documenting common keys, optional keys (`random_seed`, `separate_outputs`), `basis` vocabulary (including `corrupted`), and `vars` dict conventions.
8. Breaking (Stats): standardized length-related stats keys across modules: `*_len` → `*_length`, `*_len_unique` → `*_length_unique_count`, and `lenstat` `len_stat` → `length_stats`.
9. Bugfix (Verify): `ExmotifConflictDetails` now uses non-overlapping positions to match `occurrences` (and baseline) semantics.
10. Breaking (Stats): expanded abbreviated stats keys for machine readability across modules (e.g. `prob_lens`→`problem_lengths`, `prob_count`→`problematic_sequence_counts`, `distance_distro`→`distance_distribution`, `num_splits`→`split_count`, `typeIIS_rc_motif`→`type_iis_reverse_complement_motif`, `oligo_limit`→`oligo_length_limit`).
11. Breaking (Stats): additional key normalization for machine readability and naming consistency, including `incalc_reads`→`incalculable_reads`, `mis_assoc_reads`→`misassociated_reads`, `meta_agg_count`→`meta_aggregate_count`, `r1_truncated`/`r2_truncated`→`has_read_1_truncated`/`has_read_2_truncated`, `crossdimer_fail`→`cross_dimer_fail`, `orphan_oligo`→`orphan_oligo_ids`, `fill_count`/`left_count`→`filled_kmer_count`/`remaining_kmer_count`, and boolean flags to `has_*` form.
12. Breaking (Stats): normalized objective and engine-emitted key names for consistency across producer/consumer boundaries, including fixes for malformed keys (`existing_distance_failure_counture_count`/`cross_distance_failure_counture_count`) and split engine alignment (`split_lens`/`num_splits`/`overlap_lens`/`mean_Tm_distro`/`mean_distance_distro` → `split_lengths`/`split_count`/`overlap_lengths`/`mean_melting_temperature_distribution`/`mean_distance_distribution`).
13. Breaking (Stats): expanded remaining thermo/composition abbreviations in stats keys: `primer_Tm`→`primer_melting_temperature`, `primer_GC`→`primer_guanine_cytosine_content`, `*_MFE`→`*_minimum_free_energy`, `phiX_reads`→`phix_reads`, and related pad/estimate key families.
14. Breaking (Stats/Warns): normalized warning payload keys for machine readability: `exmotif_prefix_group`/`exmotif_suffix_group`→`excluded_motif_prefix_group`/`excluded_motif_suffix_group`, `exmotif_embedded`→`excluded_motif_embedded`, `palindrome_embedded`→`embedded_palindromes`, and index warning vars `expectedcount`/`associatecount`/`associateunique`→`expected_count`/`associate_count`/`associates_unique`.
15. Breaking (QC/Verify): renamed verify stats keys `exmotif_conflict_count`/`exmotif_column_conflicts` to `excluded_motif_conflict_count`/`excluded_motif_column_conflicts`; updated smoketests/docs accordingly.
16. Breaking (Stats): normalized remaining boolean stats keys to explicit prefixes: `limit_overflow`→`has_limit_overflow`, `callback_error`→`has_callback_error`, and `read_1_truncated`/`read_2_truncated`→`has_read_1_truncated`/`has_read_2_truncated`.
17. Breaking (Stats): normalized mixed-case Type IIS stats keys in pad module to lowercase snake_case: `typeIIS_motif`→`type_iis_motif`, `typeIIS_reverse_complement_motif`→`type_iis_reverse_complement_motif`, `internal_typeIIS_sites`→`internal_type_iis_sites`, `typeIIS_free`→`type_iis_free`.
18. Docs (`docs/api.md`): updated stats-key naming guidance to prefer expanded lowercase terms (instead of preserving acronym-style keys).
19. Docs: fixed stale `any_conflict` refs → `any_conflict_count` in `docs/docs.md`; updated `cross_barcode_columns` type from `str | list | None` to `str | iterable[str] | None` in `docs/api.md`.

2026.02.17
1. Chore: version bump to `v2026.02.17`.
2. Feature (QC/Verify): `verify` now provides column-level conflict attribution — when constituent DNA columns are present (with or without `CompleteOligo`), each conflict position is mapped to the originating column; stats include `excluded_motif_column_conflicts`, `background_column_conflicts`, and `constituent_columns`. When only `CompleteOligo` is present, all positions attribute to `'CompleteOligo'`. No signature changes.
3. Breaking (QC/Verify): conflict detail dicts and stats keys restructured for machine readability. Detail dicts — `LengthConflictDetails`: `length`→`oligo_length`, `limit`→`length_limit`, `overflow`→`excess_bp`; new `column_lengths` dict. `ExmotifConflictDetails`: changed from motif-keyed dict to list of fixed-schema dicts with explicit `motif` key; `count`→`occurrences`, `baseline`→`library_baseline`, `emergence`→`excess_occurrences`; new `columns` list parallel to `positions`. `BackgroundConflictDetails`: `matching_kmers`→`kmers`, `kmer_count`→`match_count`; removed `first_match_position` (use `positions[0]`); new `positions` and `columns` lists parallel to `kmers`. Stats vars — `complete_oligo`→`has_complete_oligo`, `oligo_limit`→`length_limit`, `min_oligo_len`→`min_oligo_length`, `max_oligo_len`→`max_oligo_length`, `length_conflict`→`length_conflict_count`, `exmotif_conflict`→`excluded_motif_conflict_count`, `background_conflict`→`background_conflict_count`, `any_conflict`→`any_conflict_count`, `motif_baselines`→`library_baselines`, `background_k`→`background_kmer_lengths`.
4. Feature (QC/Verify): integrity conflicts (`CompleteOligo` vs constituent mismatch, `OligoLength` mismatch) are now first-class DataFrame columns: `HasIntegrityConflict` (bool) and `IntegrityConflictDetails` (dict with `complete_oligo_conflict`, `oligo_length_conflict`, `expected_oligo_length`, `actual_oligo_length`). Integrity feeds into `HasAnyConflicts`. Stats keys consolidated: `integrity_conflict_count`, `complete_oligo_conflict_count`, `oligo_length_conflict_count` (replaces old `consistency_conflict_count`/`consistency_conflict_ids`/`oligolength_conflict_count`/`oligolength_conflict_ids`).
5. Bugfix (Design Mode): context-column adjacency check and context-sequence extraction now skip over previously-designed output columns (`barcode_column`, `spacer_column`, `motif_column`, `primer_column`, and `cross_barcode_columns`) so that multi-step design pipelines no longer falsely reject adjacent context columns or contaminate flanking-sequence concatenation when a designed column sits between the left and right contexts in the DataFrame.
6. Bugfix (Barcode): moved `cross_cols` string-to-list normalization before `skip_cols` construction so a single-string `cross_cols` is no longer iterated character-by-character when building the adjacency skip set.
7. Style: unified `isinstance` guard in `barcode.py` skip-cols construction to match `motif.py`, `spacer.py`, and `primer.py`; added missing `skip_cols` docstring entry to `get_parsed_column_info`.
8. Style (Barcode): `cross_cols` skip-set guard now uses `hasattr(__iter__)` iterable detection (matching `get_parsed_exmotifs` pattern) instead of `isinstance(list, tuple)`.
9. Style (Validation): `get_parsed_cross_barcode_info` now materializes any iterable `crosscols` input early via `hasattr(__iter__)`, matching the `get_parsed_exmotifs` pattern; downstream `isinstance` checks simplified to `list`.
10. Docs: updated stale `LevelDB` references to `ShareDB` across `__init__.py`, `README.md`, `docs/docs.md`, `docs/api.md`, and `vectordb.py`.
11. Docs (`docs/agent-skills.md`): clarified that the `ID` column is a literal required column name, not a conceptual description.
12. Fix (Liner): `liner_engine` pipe mode now buffers the last ephemeral line and flushes it with `\n` when a `|*` retention tag arrives, so pipe consumers see the retained line just as TTY users do; made `barcode.py` "Preparing" progress lines ephemeral (removed stale `\n`).
13. Style (Motif): fixed stale comments in `motif.py` — "Step 2 Warning" → "Step 3 Warning", "Parse primerseq" → "Parse motifseq".
14. Style (Barcode): deferred Existing Set Size and Cross Set Size printouts so both right-align via shared `plen`; colons already aligned at the same column.
