...
Point your AI coding assistant to [`docs/agent-skills.md`](docs/agent-skills.md) for optimized workflows and best practices. Claude Code and Cursor can read it directly from the repo; for ChatGPT or Claude.ai, share or upload the file. (CLI note: `op` intentionally has no `--help`; use `op COMMAND` / `op manual COMMAND`.)
